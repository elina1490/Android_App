  ...
  implements BluetoothConnectionListener, Component, Deleteable
{
  ...
  private void handleError(String paramString, int paramInt)
  {
    ...
    arrayOfObject[0] = ("Error code 0x" + Integer.toHexString(paramInt & 0xFF));
    localForm.dispatchErrorOccurredEvent(this, paramString, 404, arrayOfObject);
  }

  private byte[] receiveReturnPackage(String paramString)
  {
    byte[] arrayOfByte1 = this.bluetooth.read(paramString, 2);
    if (arrayOfByte1.length == 2)
    {
      int i = getUWORDValueFromBytes(arrayOfByte1, 0);
      byte[] arrayOfByte2 = this.bluetooth.read(paramString, i);
      if (arrayOfByte2.length >= 3)
        return arrayOfByte2;
    }
  ...
  protected final void copySLONGValueToBytes(int paramInt1, byte[] paramArrayOfByte, int paramInt2)
  {
    ...
    int k = j >> 8;
    paramArrayOfByte[(paramInt2 + 3)] = ((byte)(k & 0xFF));
  }
  ...
  protected final void copyStringValueToBytes(String paramString, byte[] paramArrayOfByte, int paramInt1, int paramInt2)
  {
    if (paramString.length() > paramInt2)
      paramString = paramString.substring(0, paramInt2);
    try
    {
      byte[] arrayOfByte2 = paramString.getBytes("ISO-8859-1");
      arrayOfByte1 = arrayOfByte2;
      System.arraycopy(arrayOfByte1, 0, paramArrayOfByte, paramInt1, Math.min(paramInt2, arrayOfByte1.length));
      return;
    }
  ...
  protected final void copyULONGValueToBytes(long paramLong, byte[] paramArrayOfByte, int paramInt)
  {
    ...
    long l1 = paramLong >> 8;
    paramArrayOfByte[(paramInt + 1)] = ((byte)(int)(l1 & 0xFF));
    long l2 = l1 >> 8;
    paramArrayOfByte[(paramInt + 2)] = ((byte)(int)(l2 & 0xFF));
    long l3 = l2 >> 8;
    paramArrayOfByte[(paramInt + 3)] = ((byte)(int)(l3 & 0xFF));
  }
  ...
  {
    return 0xFF & paramArrayOfByte[paramInt] | paramArrayOfByte[(paramInt + 1)] << 8;
  }
  ...
  protected final int getStatus(String paramString, byte[] paramArrayOfByte, byte paramByte)
  {
    if (paramArrayOfByte.length >= 3)
    {
      if (paramArrayOfByte[0] != 2)
        Log.w(this.logTag, paramString + ": unexpected return package byte 0: 0x" + Integer.toHexString(0xFF & paramArrayOfByte[0]) + " (expected 0x02)");
      if (paramArrayOfByte[1] != paramByte)
        Log.w(this.logTag, paramString + ": unexpected return package byte 1: 0x" + Integer.toHexString(0xFF & paramArrayOfByte[1]) + " (expected 0x" + Integer.toHexString(paramByte & 0xFF) + ")");
  ...
  protected final String getStringValueFromBytes(byte[] paramArrayOfByte, int paramInt)
  {
    for (int i = paramInt; ; i++)
    {
      ...
      if (i < j)
      {
        ...
        return getStringValueFromBytes(paramArrayOfByte, paramInt, k);
    }
  }
  ...
  {
    int i = sanitizePower(paramInt2);
    byte[] arrayOfByte = new byte[12];
    arrayOfByte[0] = -128;
    arrayOfByte[1] = 4;
    copyUBYTEValueToBytes(paramInt1, arrayOfByte, 2);
    copySBYTEValueToBytes(i, arrayOfByte, 3);
    copyUBYTEValueToBytes(paramInt3, arrayOfByte, 4);
 ...
 * Qualified Name:     com.google.appinventor.components.runtime.LegoMindstormsNxtBase
